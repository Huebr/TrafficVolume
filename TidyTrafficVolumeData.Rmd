...
for(i in seq_along(valid_dates)){
  ...
  write_csv(filter(myresult,Date == instance),paste("./data/output_tests/daily/",valid_dates[i],".csv",sep=""))
}
```
Criando teste multiperiodo primeiro vamos criar id's unicos para Segemnto por direção
```{r}
library(sqldf)
myresult<-myresult %>% mutate(newID=group_indices(.,`Segment ID`,Direction))
sqlStmt <- "select Date, count(distinct newID) from myresult group by Date"
sqldf(sqlStmt)
myresult %>% select(c("Roadway Name","From","To","newID")) %>% arrange(newID) %>% distinct

```


```{r}
idtable<-myresult %>% select(c("Roadway Name","From","To","newID")) %>% arrange(newID) %>% distinct
```
