...
for(i in seq_along(valid_dates)){
  ...
  write_csv(filter(myresult,Date == valid_dates[i]),paste("./data/output_tests/daily/",valid_dates[i],".csv",sep=""))
}
```
  ...
  for(p in meus_ps){
    for(i in seq_along(valid_dates)){
      ...
      for(partition in 1:nrow(m)){
         st<-paste("./data/output_tests/new_instances/",paste(valid_dates[i],k*100,p*100,number_of_locations,paste(m[partiion,],collapse = "-"),sep = "-"),".gzip",sep="")
       print(st)
       #create instance
       ts<-create_instance(number_of_locations*k,1,filter(myresult,Date == valid_dates[i]),m[partition,],p)
       n3 <- dim(ts)[3]
